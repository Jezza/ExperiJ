...
public final class Skip {
	...
	public static Predicate<String> createFilter(String rule, BinaryOperator<Predicate<String>> merge) {
		...
		for (String item : split) {
			...
			if (item.startsWith(STARTS_WITH)) {
				String with = convert(item, STARTS_WITH.length());
				_filter = input -> input.startsWith(with);
			} else if (item.startsWith(ENDS_WITH)) {
				String with = convert(item, ENDS_WITH.length());
				_filter = input -> input.endsWith(with);
			} else {
				String with = convert(item, 0);
				_filter = input -> input.contains(with);
			}
	...
	private static String convert(String in, int offset) {
		int length = in.length();
		StringBuilder out = new StringBuilder(length - offset);
		int index;
		while ((index = in.indexOf('.', offset)) != -1) {
			out.append(in, offset, index).append('/');
			offset = index + 1;
		}
		out.append(in, offset, length);
		return out.toString();
	}
}
